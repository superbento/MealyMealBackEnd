...
/**
 * @param $mealName
 * @param $cal_restriction
 * @return array
 */
function findAliment($mealName, $cal_restriction){
    GLOBAL $co;
    $incertitude = $cal_restriction * 0.05;
    $aliments = array();
    $minimumcal = 1500/4  ; //1200 for woman
                          //1800 for man
    ...
    switch ($mealName){
        case 'breakfast':
            ...
            if( $cal > ($cal_restriction + $incertitude) ){
            ...
            }else if($cal >= $minimumcal && $cal <= ($cal_restriction + $incertitude)){
                ...
                // echo "cccc "+$nb2;
                $al2 = $result[$nb2];
                $missingCalories = $minimumcal - $cal;
        ...
        case 'dish':
            ...
            if($mealCal > ($cal_restriction + $incertitude)){
                $coefficient = $cal_restriction/$mealCal;
                $quantityMeat = $meatCal/($meat['calories_per_100g']/100)*$coefficient;
                $quantitySidedish = $sidedishCal/($sidedish['calories_per_100g']/100)*$coefficient;
                $quantityVegetable = $vegetableCal/($vegetable['calories_per_100g']/100)*$coefficient;

            }else if($mealCal >= $minimumcal && $mealCal <= ($cal_restriction + $incertitude)){
                ...
                $coefficient = $mealCal/$minimumcal;
                $quantityMeat = $meatCal/($meat['calories_per_100g']/100)*$coefficient;
                $quantitySidedish = $sidedishCal/($sidedish['calories_per_100g']/100)*$coefficient;
                $quantityVegetable = $vegetableCal/($vegetable['calories_per_100g']/100)*$coefficient;
            }
        ...
        case 'snack':
            ...
            if( $cal > ($cal_restriction + $incertitude)){
            ...
            }else if($cal >= ($minimumcal/2) && $cal <= ($cal_restriction + $incertitude)){
                ...
                // echo "cccc "+$nb2;
                $al2 = $result[$nb2];
                $missingCalories = ($minimumcal/2) - $cal;
                $quantity2 = $missingCalories/($al2['calories_per_100g']/100);
